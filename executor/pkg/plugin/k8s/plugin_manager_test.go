...
func TestShouldReadTaskOutputs(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "no interface", template: &core.TaskTemplate{}, expectedDecision: false},
		{
			name: "empty outputs",
			template: &core.TaskTemplate{Interface: &core.TypedInterface{
		...
		},
		{
			...
			template: &core.TaskTemplate{Interface: &core.TypedInterface{
				Outputs: &core.VariableMap{Variables: []*core.VariableEntry{{Key: "result", Value: &core.Variable{}}}},
			}},
			expectedDecision: true,
		},
		{name: "buffered plugin output", bufferedOutput: true, expectedDecision: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tCtx := pluginsCoreMock.NewTaskExecutionContext(t)
			pCtx := &pluginContext{}
			if tt.bufferedOutput {
				pCtx.ow = &ioutils.BufferedOutputWriter{}
			} else {
				taskReader := pluginsCoreMock.NewTaskReader(t)
